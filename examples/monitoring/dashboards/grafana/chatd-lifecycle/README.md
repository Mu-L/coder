...
`coderd_chatd_stage_duration_seconds{stage,scope,chat_kind,organization_name}`
...
`coderd_chatd_model_stage_duration_seconds{stage,provider,chat_kind,organization_name,model}`.
...
where the model explains the duration. Worst case, when every stage
occurs in every tuple, per replica:

| Level   | Per `(chat_kind, organization_name)`                         | Per `(chat_kind, organization_name, model)` |
|---------|--------------------------------------------------------------|---------------------------------------------|
| `basic` | ~180 (11 stages x 15 series, plus 16 partition counters)     | 45 (3 stages x 15 series)                   |
| `full`  | ~930 (15 stages x 19, plus the three per-turn distributions) | 95 (5 stages x 19)                          |

A histogram series set is its bucket edges plus `+Inf`, `_sum`, and
`_count`. Single-organization deployments pay nothing for the
organization label. For scale, a coderd replica exposes roughly 3,400
`coderd_*` series on its own before chat instrumentation; a
deployment with two organizations, both chat kinds, and five models
in use per organization adds about 1,600 series per replica at
`basic` and about 5,600 at `full`. Series only appear for tuples that
see traffic.
  ...
  - name: chatd-latency
    rules:
      ...
      - alert: ChatdSlowTimeToFirstToken
        expr: |
          1 - (
            sum by (provider, model) (rate(coderd_chatd_model_stage_duration_seconds_bucket{stage="time_to_first_token", le=~"10(\\.0)?"}[10m]))
            /
            sum by (provider, model) (rate(coderd_chatd_model_stage_duration_seconds_count{stage="time_to_first_token"}[10m]))
          ) > 0.10
          and
          sum by (provider, model) (increase(coderd_chatd_model_stage_duration_seconds_count{stage="time_to_first_token"}[10m])) > 20
        for: 5m
...
`chat_turn` is kept because the dashboard's chat kind and organization
variables are sourced from it, `time_to_first_token` on the model
histogram because the model variable is, and `capacity_wait` so an
alert on capacity wait time can be written against it. With the third
rule in place the Turn time partition row and the collapsed rows are
empty.
...
The stage histogram carries four labels and the model histogram five,
...
| `organization_name` | both, and the turn families    | name of the organization that owns the chat, empty for stages recorded without a known chat             | `$organization` (multi-select) |
| `provider`          | `model_stage_duration_seconds` | provider the model was resolved through                                                                 | none (panels group by it)      |
| `model`             | `model_stage_duration_seconds` | resolved model ID                                                                                       | `$model` (multi-select)        |

Title generation runs without a chat: its background `provider_attempt`
carries empty `chat_kind` and `organization_name`. Summary and
status-label generation run for a chat and carry that chat's kind and
organization.
...
therefore filters every stage panel in full. `organization_name` is
likewise a property of the turn and `$organization` filters the same
panels; it is applied wherever `$chat_kind` is. Background provider
...
`chat_kind` and `organization_name` only. The first model a turn
resolves is an attribute on its `chat_turn` span.
...
this row does not see. Dimensions: `$chat_kind` and `$organization`
apply.
...
and `$organization` apply; the grouping is fixed to `chat_kind`, because
...
The panels that read `coderd_chatd_model_stage_duration_seconds`, the
one stage family labeled by `provider` and `model`. `$chat_kind`,
`$organization`, and `$model` all apply. The pre-existing
`coderd_chatd_ttft_seconds{provider,model}` histogram measures the same
first-token interval without the chat labels and is not used by the
dashboard.
...
**Provider attempts per minute by provider and model** - rate of
...
and `$organization` apply, `$model` does not. Summary and status-label generation
carry the kind and organization of the chat they ran for; title
generation runs without a chat and carries empty `chat_kind` and
`organization_name`, so it is shown while those variables are set to All
and drops out once a kind or organization is selected. How to read: this
