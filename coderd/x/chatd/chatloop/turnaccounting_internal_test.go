...
func TestTurnAccountingPartition(t *testing.T) {
	...
	// The turn families carry the chat identity but not the model: a
	// turn is not a model-scoped unit.
	labels := fixture.labelsOf(t, "coderd_chatd_turn_stage_seconds", "stage", StageStream)
	require.NotContains(t, labels, "model")
	require.NotContains(t, labels, "effort")
	require.Equal(t, ChatKindRoot, labels["chat_kind"])
	require.Equal(t, "acme", labels["organization_name"])
	labels = fixture.labelsOf(t, "coderd_chatd_turn_time_seconds", "category", CategoryStreaming)
	require.NotContains(t, labels, "model")
}
...
func TestTurnMetricsLevels(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(string(tt.level), func(t *testing.T) {
			...
			metrics.RecordTurnStage(StageStream, ChatKindRoot, "acme", time.Minute)
			metrics.RecordTurn(ChatKindRoot, "acme")
			metrics.RecordTurnOutcome(TurnOutcomeCompleted, ChatKindRoot, "acme")
			metrics.RecordTurnOutcome(TurnOutcomeError, ChatKindRoot, "acme")
			metrics.RecordTurnCategory(CategoryStreaming, ChatKindRoot, "acme", time.Minute, 0.5)
			metrics.RecordTurnCategory(CategoryScheduling, ChatKindRoot, "acme", 0, 0)
			...
			for _, metric := range byName["coderd_chatd_turn_time_seconds_total"].GetMetric() {
				for _, label := range metric.GetLabel() {
					if label.GetName() == "category" {
						seconds[label.GetValue()] = metric.GetCounter().GetValue()
					}
				}
			}
			require.Equal(t, map[string]float64{CategoryStreaming: 60, CategoryScheduling: 0}, seconds)
			...
			require.Equal(t, map[string]float64{TurnOutcomeCompleted: 1, TurnOutcomeError: 1}, outcomes)
		})
	}
}
