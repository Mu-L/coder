...
func stageSampleCount(t *testing.T, registry *prometheus.Registry, stage string) (uint64, bool) {
	t.Helper()
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			if metricLabel(metric, "stage") == stage {
...
func modelStageSeries(t *testing.T, registry *prometheus.Registry, stage string) (*dto.Metric, bool) {
	t.Helper()
	families, err := registry.Gather()
	require.NoError(t, err)
	for _, family := range families {
...
func endedSpan(t *testing.T, spans *tracetest.SpanRecorder, stage string) sdktrace.ReadOnlySpan {
	t.Helper()
...
func TestGuardedStreamTTFTStage(t *testing.T) {
	...
	t.Run("FirstPartObservesStage", func(t *testing.T) {
		...
		attempt, err := guardedStream(
			...
			fixture.metrics, fixture.tracer, StageModel{Provider: "anthropic", Model: "claude", Effort: "high"},
		)
		require.NoError(t, err)
		parts := 0
		...
		require.Equal(t, "anthropic", metricLabel(modelSeries, "provider"))
		require.Equal(t, "claude", metricLabel(modelSeries, "model"))
		require.Equal(t, uint64(1), ttftSampleCount(t, fixture.registry))
		require.Equal(t, codes.Unset, endedSpan(t, fixture.spans, StageTimeToFirstToken).Status().Code)
	})
