...
const (
	...
	CategoryPreparation      = "preparation"
...
// addStage adds the time one occurrence of a stage took. A stage that
// took no measurable time still gets an entry, so the turn reports it.
...
func (s *StageSpan) report(elapsed time.Duration, err error) {
	...
	if s.stage == StageChatTurn {
		s.tracer.emitTurnAccounting(s.acc, s.chatKind, s.organization, elapsed)
		return
	}
	if s.node == nil {
		return
...
func (t *StageTracer) RecordTurnOutcome(outcome, chatKind, organization string) {
	if t == nil || t.metrics == nil {
		return
	}
	t.metrics.RecordTurnOutcome(outcome, chatKind, organization)
}
...
func (t *StageTracer) emitTurnAccounting(acc *TurnAccumulator, chatKind, organization string, turnDuration time.Duration) {
	if t == nil || t.metrics == nil {
		return
	}
	...
	t.metrics.RecordTurnOutcome(TurnOutcomeCompleted, chatKind, organization)
	if turnDuration <= 0 {
	...
	for stage, elapsed := range snapshot.stages {
		t.metrics.RecordTurnStage(stage, chatKind, organization, elapsed)
	}
	...
	t.metrics.RecordTurn(chatKind, organization)
	for _, category := range TurnTimeCategories {
		elapsed := snapshot.categories[category]
		t.metrics.RecordTurnCategory(category, chatKind, organization, elapsed, elapsed.Seconds()/turnSeconds)
	}
}
