	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			key := stageKey{
				...
				org:      labelValue(metric, "organization_name"),
			}
...
func (f stageFixture) modelStageObservations(t *testing.T) map[modelStageKey]uint64 {
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			key := modelStageKey{
				stage:    labelValue(metric, "stage"),
				provider: labelValue(metric, "provider"),
				chatKind: labelValue(metric, "chat_kind"),
				org:      labelValue(metric, "organization_name"),
...
func TestStageTracerChatKind(t *testing.T) {
	...
	t.Run("InheritedByDerivedStages", func(t *testing.T) {
		...
		require.Equal(t, map[stageKey]uint64{
			{stage: chatloop.StageChatTurn, scope: chatloop.ScopeTurn, chatKind: chatloop.ChatKindSubagent, org: "acme"}:       1,
			{stage: chatloop.StageGenerationStep, scope: chatloop.ScopeTurn, chatKind: chatloop.ChatKindSubagent, org: "acme"}: 1,
			{stage: chatloop.StageToolCall, scope: chatloop.ScopeTurn, chatKind: chatloop.ChatKindSubagent, org: "acme"}:       1,
		}, fixture.stageObservations(t))
...
func TestStageTracerModelLabels(t *testing.T) {
	...
	model := chatloop.StageModel{Provider: "anthropic", Model: "claude-sonnet-4-5", Effort: "high"}
...
func TestStageDurationBuckets(t *testing.T) {
	...
	tests := []struct {
		...
		{level: codersdk.ChatStageMetricsLevelBasic, wantBuckets: 12},
		{level: codersdk.ChatStageMetricsLevelFull, wantBuckets: 16},
	}
	...
	for _, tt := range tests {
		...
		metrics.RecordStageDuration(chatloop.StageStream, chatloop.ScopeTurn, chatloop.ChatKindRoot, "",
			chatloop.StageModel{Provider: "p", Model: "m"}, 45*time.Minute)
		...
		require.Len(t, buckets, tt.wantBuckets, "level %s", tt.level)
		// Both histograms use the same ladder at a level.
		require.Len(t, modelBuckets, tt.wantBuckets, "level %s", tt.level)
		for i := range buckets {
			require.Equal(t, buckets[i].GetUpperBound(), modelBuckets[i].GetUpperBound())
		}
		...
		for _, want := range alertEdges {
			require.Contains(t, edges, want, "level %s: bucket edge %v missing", tt.level, want)
		}
		// A 45 minute turn lands in the 1800-3600 bucket, not the overflow.
		for _, bucket := range buckets {
			if bucket.GetUpperBound() < 3600 {
				require.Zero(t, bucket.GetCumulativeCount(), "level %s: le=%v", tt.level, bucket.GetUpperBound())
			} else {
				require.Equal(t, uint64(1), bucket.GetCumulativeCount(), "level %s: le=%v", tt.level, bucket.GetUpperBound())
			}
		}
		switch tt.level {
		case codersdk.ChatStageMetricsLevelBasic:
			basicEdges = edges
		case codersdk.ChatStageMetricsLevelFull:
...
func TestStageMetricsLevels(t *testing.T) {
	...
	model := chatloop.StageModel{Provider: "p", Model: "m"}
	...
	basicStages := []string{chatloop.StageQueueWait, chatloop.StageTimeToFirstToken}
	fullStages := []string{chatloop.StagePrepare, chatloop.StageQueueWait, chatloop.StageThinking, chatloop.StageTimeToFirstToken}
	basicModelStages := []string{chatloop.StageTimeToFirstToken}
	fullModelStages := []string{chatloop.StageThinking, chatloop.StageTimeToFirstToken}
	tests := []struct {
		level           codersdk.ChatStageMetricsLevel
		wantLevel       string
		wantStages      []string
		wantModelStages []string
		wantFamily      bool
	}{
		...
		// Case is ignored; unknown and empty values fall back to basic.
		{
			level: "FULL", wantLevel: "full", wantFamily: true,
			wantStages: fullStages, wantModelStages: fullModelStages,
		},
		{
			level: "", wantLevel: "basic", wantFamily: true,
			wantStages: basicStages, wantModelStages: basicModelStages,
		},
		{
			level: "verbose", wantLevel: "basic", wantFamily: true,
			wantStages: basicStages, wantModelStages: basicModelStages,
		},
	}
	for _, tt := range tests {
		t.Run(string(tt.level), func(t *testing.T) {
			...
			// The gauge is one-hot: every level has a series and only the
			// configured one reads 1.
			level := byName["coderd_chatd_stage_metrics_level"]
			require.NotNil(t, level)
			levels := map[string]float64{}
			for _, metric := range level.GetMetric() {
				levels[labelValue(metric, "level")] = metric.GetGauge().GetValue()
			}
			wantLevels := map[string]float64{}
			for _, value := range codersdk.ChatStageMetricsLevelValues {
				wantLevels[value] = 0
			}
			wantLevels[tt.wantLevel] = 1
			require.Equal(t, wantLevels, levels)
			...
			require.Equal(t, tt.wantFamily, hasModelDurations)
			...
			for _, metric := range byName["coderd_chatd_model_stage_duration_seconds"].GetMetric() {
				modelStages = append(modelStages, labelValue(metric, "stage"))
				require.Equal(t, model.Provider, labelValue(metric, "provider"))
