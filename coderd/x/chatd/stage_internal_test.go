...
func TestStageSpanRoundTripper(t *testing.T) {
	...
	model := chatloop.StageModel{Provider: "anthropic", Model: "claude-sonnet-4-5", Effort: "medium"}
	tests := []struct {
		name           string
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			span := ended[0]
			require.Equal(t, chatloop.StageProviderAttempt, span.Name())
			...
			require.Contains(t, span.Attributes(), attribute.String(chatloop.AttrScope, wantScope))
			...
			require.Contains(t, span.Attributes(), attribute.String(chatloop.AttrProvider, model.Provider))
...
func TestNewModelTracesEachProviderAttempt(t *testing.T) {
	...
	stageModel := chatloop.StageModel{Provider: "openai", Model: "gpt-4", Effort: "low"}
	...
	for _, span := range ended {
		require.Equal(t, chatloop.StageProviderAttempt, span.Name())
		require.Contains(t, span.Attributes(), attribute.String(chatloop.AttrProvider, stageModel.Provider))
...
func turnStageTurns(t *testing.T, registry *prometheus.Registry) map[string]uint64 {
	t.Helper()
...
func emittedTurns(t *testing.T, registry *prometheus.Registry) uint64 {
	...
	for _, family := range families {
		...
		for _, metric := range family.GetMetric() {
			for _, label := range metric.GetLabel() {
				if label.GetName() == "category" && label.GetValue() == chatloop.CategoryUnattributed {
...
func TestRunnerTurnSpanCarriesChatIdentity(t *testing.T) {
	...
	for _, span := range ended {
		...
		require.Contains(t, span.Attributes(),
			attribute.String(chatloop.AttrChatKind, chatloop.ChatKindSubagent), span.Name())
	}
}
...
// outcome.
func turnOutcomeCounts(t *testing.T, registry *prometheus.Registry) map[string]float64 {
	t.Helper()
	families, err := registry.Gather()
	require.NoError(t, err)
...
func TestRunnerTurnSpanEnsureRotatesInvalidatedTurn(t *testing.T) {
	...
	require.Equal(t, chatloop.TurnOutcomeError, outcome.AsString())
	require.Equal(t, secondTrigger.UTC(), turns[1].StartTime().UTC())
	require.Equal(t, codes.Unset, turns[1].Status().Code)
	outcome, _ = spanAttribute(t, turns[1], chatloop.AttrTurnOutcome)
	require.Equal(t, chatloop.TurnOutcomeCompleted, outcome.AsString())
}
...
func TestRunnerTurnSpanOutcome(t *testing.T) {
	...
	outcome := func(t *testing.T, span sdktrace.ReadOnlySpan) string {
		t.Helper()
