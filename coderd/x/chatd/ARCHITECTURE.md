...
The runner owns a `chat_turn` trace span for the turn it is running, implemented by `runnerTurnSpan` in `turn_trace.go`. The span and the stages inside it are emitted through `chatloop.StageTracer`, which produces an OpenTelemetry span and an observation on the `coderd_chatd_stage_duration_seconds{stage, scope, chat_kind, organization_name}` histogram from a single `End` call, so wherever both exist the trace and metric durations cannot disagree. The stages that are the provider's work on a model (`time_to_first_token`, `stream`, `provider_attempt`, and at `full` also `thinking` and `compaction`) are observed a second time on `coderd_chatd_model_stage_duration_seconds{stage, provider, chat_kind, organization_name, model}`, the only stage family that carries the model, so the series count scales with the number of models only where the model explains the duration. Tracing is enabled by the `TracerProvider` server option. A nil provider disables spans without disabling the histogram. The `--chat-stage-metrics` option (`off`, `basic`, `full`; default `basic`) filters the histogram without affecting spans: `basic` observes only the wait, connect, and model-call stages on a 12-bucket ladder, `full` observes every stage on a 16-bucket ladder, and `off` registers none of the stage families. `coderd_chatd_stage_metrics_level` reports the configured level.
...
- Start: every iteration of the generation loop calls `Ensure`, which returns a context parented to the open turn and a `turnToken` identifying it. When no turn is open, `Ensure` starts one with its start timestamp backdated to the trigger time and records an `acquisition` stage from that instant to now, covering the time between the trigger and a worker picking the chat up. The trigger time is the later of the last user prompt's `created_at` and the chat's `compaction_requested_at`, so a turn started by `/compact` is anchored at the request rather than at the prompt before it. The anchor never precedes the anchor of the previous turn on the same runner: an older trigger is clamped to that anchor and counted under `coderd_chatd_stage_anomalies_total{reason="stale_anchor"}`. A prompt that lands while the previous turn is still running keeps its own trigger as the anchor.
- Invalidate: a step that fails calls `Invalidate` with an outcome and the error. The first call is kept; the span stays open until the next `Ensure` on the runner closes it or the runner exits. The outcome is `interrupted` when the task context is done, `abandoned` for an expected exit without cancellation (a fence or history mismatch), and `error` otherwise. A retryable error does not invalidate the turn; the retry continues it.
...
Every stage carries `scope` (`turn` or `background`), `chat_kind` (`root` or `subagent`), and `organization_name` (the chat's organization, resolved once per turn and cached for the life of the server). Stages recorded without a known chat carry empty `chat_kind` and `organization_name` labels. Spans additionally carry `provider`, `model`, and `reasoning_effort` once the model is resolved; `prepare` is stamped with them when preparation resolves the model. Of these only `provider` and `model` are metric labels, and only on `coderd_chatd_model_stage_duration_seconds`; `reasoning_effort` multiplies series per model without changing how the stages behave.
...
- `time_to_first_token`: nested in `stream`, from opening the request to the first streamed part. It is observed for every attempt that produced a part or failed; a failed attempt, including one whose first part is an error part, ends it with the error. A stream released before any part arrives and without a failure closes this span with an error and records no histogram observation.
...
When a turn finishes normally, its stages are rolled up and emitted once, labelled with the turn's `chat_kind` and `organization_name`. The turn families carry no `model` label: a turn is not a model-scoped unit (subagents, compaction summaries, and model switches can run inside one), and per-model time is available from `coderd_chatd_model_stage_duration_seconds`. The first model resolved in the turn is stamped on the `chat_turn` span as an attribute. Turns that end interrupted, in error, or abandoned are invalidated and emit nothing.

Every closed turn is counted exactly once in `coderd_chatd_turn_outcomes_total{outcome, chat_kind, organization_name}`, registered at every level except `off`, so the outcomes sum to the closed turns. `completed` is counted when a finished turn's accounting is emitted, so it equals `coderd_chatd_turns_total` except for the turns counted under `nonpositive_turn`; `interrupted`, `error`, and `abandoned` are counted when the turn closes, with the same value as the span's `turn_outcome` attribute.

Per stage, `coderd_chatd_turn_stage_seconds` records the total seconds the turn spent in that stage. Stages overlap in wall time (a `provider_attempt` runs inside `time_to_first_token`, which runs inside `stream`, which runs inside `generation_step`), so these totals do not partition the turn. Per-turn occurrence counts and per-stage shares are a trace question: the `chat_turn` span's children carry them without a series per stage per tuple. This family, `coderd_chatd_turn_time_seconds`, and `coderd_chatd_turn_time_share` are registered only at `--chat-stage-metrics=full`; the accumulator still runs at every level, so the category counters and the anomaly counter below are unaffected by the level.

The turn's wall time is partitioned into disjoint categories that sum to the turn duration. At every level except `off`, `coderd_chatd_turn_time_seconds_total` accumulates seconds per category and `coderd_chatd_turns_total` counts the accounted turns, so their ratio is mean seconds per turn by category. At `full`, `coderd_chatd_turn_time_seconds` and `coderd_chatd_turn_time_share` additionally record the per-turn distribution of each category's seconds and share. Every category is emitted for every accounted turn, including the ones with no time, so shares are comparable across turns. The categories and the stages that feed them:
