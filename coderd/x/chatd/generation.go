...
func (s *taskStarter) StartGeneration(ctx context.Context, input chatWorkerTaskStartInput) error {
	...
	for {
		...
		if err != nil && !errors.Is(err, errTaskRetryable) {
			// A retryable error leaves the turn open for the retry.
...
func turnOutcomeForError(ctx context.Context, err error) string {
	switch {
	case ctx.Err() != nil:
...
func (s *taskStarter) generateCompaction(
	...
	compactionSpan.SetModel(compactionStageModel(prepared, metricProvider, metricModel))
...
// provider and model. The effort carries over only when the chat model
// runs the summary; an override resolves its own call options, which
// this call site does not see.
func compactionStageModel(prepared generationPrepared, summaryProvider, summaryModel string) chatloop.StageModel {
	model := chatloop.StageModel{Provider: summaryProvider, Model: summaryModel}
