...
func TestRecordThinkingStages(t *testing.T) {
	...
	prepared := generationPrepared{
		...
		StageModel:       chatloop.StageModel{Model: "claude", Effort: "high"},
	}
	...
	t.Run("PairsByIndex", func(t *testing.T) {
		...
		for _, span := range ended {
			require.Equal(t, chatloop.StageThinking, span.Name())
...
func TestCompactionStageModel(t *testing.T) {
	t.Parallel()
	...
	t.Run("ChatModelKeepsEffort", func(t *testing.T) {
		t.Parallel()
		prepared := prepared
		prepared.Compaction = &generationCompaction{}
		require.Equal(t, chatloop.StageModel{Provider: "anthropic", Model: "claude", Effort: "high"},
			compactionStageModel(prepared, "anthropic", "claude"))
	})

	t.Run("OverrideDropsEffort", func(t *testing.T) {
		t.Parallel()
		prepared := prepared
		prepared.Compaction = &generationCompaction{Override: &resolvedCompactionOverride{ResolvedModel: "gpt-5"}}
		require.Equal(t, chatloop.StageModel{Provider: "openai", Model: "gpt-5"},
			compactionStageModel(prepared, "openai", "gpt-5"))
	})

	t.Run("NoCompactionDropsEffort", func(t *testing.T) {
		t.Parallel()
		require.Equal(t, chatloop.StageModel{Provider: "anthropic", Model: "claude"},
			compactionStageModel(prepared, "anthropic", "claude"))
	})
}
